          ...
          <section class="content-area">
            <div class="welcome-card">
              ...
              <div class="name-preview-card">
                ...
                <div class="name-preview-value">${escapeHtml(buildDisplayName())}</div>
              </div>
    ...
    root.innerHTML = `
      <div class="page-shell">
        <div class="card login-card">
          ...
          <div class="info-row">
            <span>数据名称</span>
            <strong>${escapeHtml(buildDisplayName())}</strong>
          </div>
...
function renderGroupTabs(current: GroupByType) {
...
function renderDetailPair(label: string, value: string) {
  return `
...
function renderDetailEditInput(
  id: string,
  label: string,
  value: string,
  type = 'text'
...
function renderDynamicFields(fields: DynamicField[]) {
  if (!fields.length) {
...
function renderStep2Rows(rows: DataItem[]) {
  return rows
    .map(
      (row) => `
        <tr>
...
function formatTestTimeForDisplay(value: string) {
  if (!value) return '';
